# backend/api/routes/supply_chain.py

import time
import logging
from fastapi import APIRouter
from backend.services.graph_service import graph_service
from backend.services.response_transformer import transform_supply_chain
...
async def get_supply_chain_network():
    ...
    response = transform_supply_chain(cache.workflow_state)
    transform_time = time.perf_counter() - transform_start
    
    api_time = time.perf_counter() - start_time
    logger.info(
        "Supply Chain Endpoint Performance: Transform: %.4fs, API: %.4fs",
        transform_time,
        api_time
    )
