...
from backend.services.response_transformer import transform_inventory
...
async def get_plants_inventory():
    ...
    response = transform_inventory(cache.workflow_state)
    transform_time = time.perf_counter() - transform_start
    
    api_time = time.perf_counter() - start_time
    logger.info(
        "Inventory Endpoint Performance: Transform: %.4fs, API: %.4fs",
        transform_time,
        api_time
    )
